...
// NewBearerCommand ...
func NewBearerCommand() *cobra.Command {
	cmd := cobra.Command{
		Use:   "bearer [OPTIONS]",
		Short: "Run a bearer token authentication server",
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			log := ctrl.Log.WithName("auth.bearer")
			s := runtime.NewScheme()
			...
			if err := bearer.RegisterWithManager(mgr); err != nil {
				return ExitErrorf(EX_FAIL, "bearer controller registration failed: %w", err)
			}
			...
			auth.RegisterServer(srv, bearer)
	...
	// Authorization flags.
	cmd.Flags().String("auth-realm", "default", "Bearer authentication realm.")
